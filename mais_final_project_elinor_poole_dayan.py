...
vectorizer = CountVectorizer(input=X_train, analyzer=clean, min_df=5, binary=True)
#vectorize X_train
...
print("Train mean accuracy", str((clf.score(X_train_vectorized, y_train)*100)) +'%')
print("Test mean accuracy", str((clf.score(X_test_vectorized, y_test)*100)) +'%')
...
def result(tweet):
  #get the prediction
  pred_cat = le.inverse_transform(clf.predict(vectorizer.transform([tweet])))[0]
